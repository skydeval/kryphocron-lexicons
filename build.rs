...
fn run() -> Result<(), BuildError> {
    ...
    write_lexicon_jsons(&out_dir, &lexicon_files)?;
...
// ============================================================
// Raw lexicon JSON emission (runtime `lexicons()` accessor).
// ============================================================

// Lightweight projection used only to recover an NSID from a
// lexicon JSON's `id` field. `parse_lexicons` already proved every
// file deserializes into a full `LexiconDoc`; here we only need the
// NSID to label the (nsid, raw-json) pair, so a minimal struct keeps
// the second pass cheap and order-independent of the full parse.
#[derive(Deserialize)]
struct LexiconIdOnly {
    id: String,
}

// Emit `lexicon_jsons.rs` into OUT_DIR: a `&[(&str, &str)]` static
// pairing each lexicon's NSID with the verbatim JSON text shipped
// under `lexicons/`. lib.rs includes it and feeds each entry to
// `Lexicons::add_from_json` in the runtime accessor. Entries are
// sorted by NSID so the generated file is deterministic across
// builds. Both fields are written via `{:?}` so arbitrary JSON
// content (quotes, backslashes, newlines) is escaped into a valid
// Rust string literal without raw-string-delimiter collisions.
fn write_lexicon_jsons(out_dir: &Path, lexicon_files: &[PathBuf]) -> Result<(), BuildError> {
    let mut pairs: Vec<(String, String)> = Vec::with_capacity(lexicon_files.len());
    for path in lexicon_files {
        let raw = fs::read_to_string(path)
            .map_err(|e| BuildError::Io(format!("read {}: {e}", path.display())))?;
        let id = serde_json::from_str::<LexiconIdOnly>(&raw)
            .map_err(|e| BuildError::LexiconParse {
                path: path.display().to_string(),
                detail: format!("recover id: {e}"),
            })?
            .id;
        pairs.push((id, raw));
    }
    ...
    let mut s = String::new();
    s.push_str("// Generated by kryphocron-lexicons/build.rs. Do not edit.\n\n");
    s.push_str("/// Verbatim `tools.kryphocron.*` lexicon JSON shipped under\n");
    s.push_str("/// `lexicons/`, paired with each doc's NSID. Consumed by the\n");
    s.push_str("/// runtime [`crate::lexicons`] accessor.\n");
    s.push_str("pub(crate) static LEXICON_JSONS: &[(&str, &str)] = &[\n");
    for (nsid, raw) in &pairs {
        s.push_str(&format!("    ({nsid:?}, {raw:?}),\n"));
    }
    s.push_str("];\n");

    let dest = out_dir.join("lexicon_jsons.rs");
    fs::write(&dest, s).map_err(|e| BuildError::Io(format!("write {}: {e}", dest.display())))?;
    Ok(())
}
